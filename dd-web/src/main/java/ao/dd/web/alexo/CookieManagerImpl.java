...
// Should only be used via the CookieManager ui.
public class CookieManagerImpl implements CookieManager
{
    ...
    private static final String     DATE_FORMAT_STRING_A =
                                        "EEE, dd-MMM-yyyy hh:mm:ss z";
    private static final String     DATE_FORMAT_STRING_B =
                                        "EEE, dd MMM yyyy hh:mm:ss z";
    private static final DateFormat DATE_FORMAT_A         =
                            new SimpleDateFormat(DATE_FORMAT_STRING_A);
    private static final DateFormat DATE_FORMAT_B         =
                            new SimpleDateFormat(DATE_FORMAT_STRING_B);


    //--------------------------------------------------------------------
    //               {Domain => {CookieName => {Key => Value}}}
    ...
    //--------------------------------------------------------------------
    /**
     * Retrieves and stores cookies returned by the host on the other
     * side of the the open java.net.URLConnection.
     * <p/>
     * The connection MUST have been opened using the <i>connect()</i>
     * method or a IOException will be thrown.
     *
     * @param conn a java.net.URLConnection - must be open, or
     *  IOException will be thrown
     * //@throws java.io.IOException Thrown if <i>conn</i> is not open.
     */
    //        {CookieName => {Key => Val}}
    public Map<String, Map<String, String>> readCookies(
            URLConnection conn)
    {
        // {CookieName => {Key => Val}}
        Map<String, Map<String, String>> domainStore =
                retrieveOrCreateDomainStore( conn.getURL() );

        // OK, now we are ready to get the cookies out of the URLConnection
        String headerName;
        for (int i = 1;
             (headerName = conn.getHeaderFieldKey(i)) != null;
             i++)
        {
            if (headerName.equalsIgnoreCase(SET_COOKIE))
            {
                ...
                // the specification dictates that the first
                //  name/value pair in the string is the cookie name and
                //  value, so let's handle them as a special case:
                if (st.hasMoreTokens())
                {
    ...
    {
        // let's determine the domain and path to retrieve
        ...
        while (cookieNames.hasNext())
        {
            ...
            // check cookie to ensure path matches and cookie is not
            //  expired if all is cool, add cookie to header string
        ...
        try
        {
            conn.setRequestProperty(
                    COOKIE, cookieStringBuffer.toString());
        }
        catch (java.lang.IllegalStateException ise)
        {
            throw new Error( /* IOException */
                    "Illegal State! Cookies cannot be set on a " +
                    "URLConnection that is already connected. Only " +
                    "call writeCookies(java.net.URLConnection) " +
                    "AFTER calling java.net.URLConnection.connect().");
        }
    }
    ...
    public void setCookies(
            ...
            Map<String, Map<String, String>> cookies)
    {
        STORE.put(getDomainFromHost(url.getHost()),
                  Cloner.clone(cookies));
    }
    ...
    private Map<String, Map<String, String>>
            retrieveOrCreateDomainStore(URL url)
    {
        return retrieveOrCreateDomainStore(
                getDomainFromHost( url.getHost() ));
    }
    private Map<String, Map<String, String>>
            retrieveOrCreateDomainStore(String domain)
    {
    ...
    private Map<String, String> retrieveOrCreateCookies(
                                    URL url, String name)
    {
        String domain = getDomainFromHost( url.getHost() );
        Map<String, Map<String, String>> domainCookies =
                retrieveOrCreateDomainStore(domain);
    ...
    private boolean isNotExpired(String cookieExpires)
    {
        ...
        catch (java.text.ParseException peA)
        {
            ...
            {
                System.out.println(
                        "Can't extract date from: " + cookieExpires);
                return false;
            }
        }
