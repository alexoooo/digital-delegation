...
public class ApacheSession implements Session
{
    ...
    private final static Logger LOG =
            Logger.getLogger(ApacheSession.class);
                       ...
                       boolean             isPost)
    {
        ...
        {
            return isPost
                    ? loadPost(address, referrer, data)
                    : loadGet (address, referrer, data);
        }
        catch (Error e)
        {
            throw e;
    ...
    //--------------------------------------------------------------------
    private String loadGet(URL                 address,
                           URL                 referer,
                           Map<String, String> data)
            throws Throwable
    {
        HttpGet method = new HttpGet(
                addEncodedData(address, data).toURI());
        //System.out.println(addEncodedData(address, data));
        setReferrer(method, referer);
        return execute(method);
    }

    private URL addEncodedData(
            URL                 url,
            Map<String, String> data)
    {
        ...
        String addend =
                ((url.toString().indexOf("?") != -1)
                  ? "&" : "?")
                + WebUtils.urlEncode(data);

        return WebUtils.asUrl(
                url.toString() + addend);
    }


    //--------------------------------------------------------------------
    private String loadPost(URL                 address,
                            URL                 referrer,
                            Map<String, String> data)
            throws Throwable
    {
    ...
    private void addPostData(HttpPost method,
                             Map<String, String> data)
    ...
    //--------------------------------------------------------------------
    // note "Referer" is misspelled on purpose, see:
    //  http://en.wikipedia.org/wiki/
    //      HTTP_referrer#Origin_of_the_term_referer
    private void setReferrer(HttpUriRequest method,
                             URL            referrer)
    {
        if (referrer != null)
        {
    ...
    //--------------------------------------------------------------------
    private String execute(final AbortableHttpRequest method)
            throws Throwable
    {
        ...
        if (! finished.await(READ_TIMEOUT, TimeUnit.MILLISECONDS))
        {
            ...
            throw new RequestError(
                    "Request took longer than " + READ_TIMEOUT);
        }

//        long start = System.currentTimeMillis();
//        while (! finished.isTrue())
//                // not .waitForTrue to allow aborting before time expires
//                // somewhy .waitForTrue(READ_TIMEOUT) doesn't work??
//        {
//            WebUtils.pause(200);
//
...
//        }
    ...
    private String doExecute(AbortableHttpRequest method)
            throws Throwable
    {
        HttpClient   client = CLIENT.newInstance();
        HttpResponse rsp    = client.execute(
