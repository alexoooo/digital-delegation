...
//    				client.client().printWorkingDirectory());
	...
	//--------------------------------------------------------------------
	@Override public ShellFile file(String remoteFilePath)
    {
		try {
			List<ShellFile> files = getFiles(
	...
	@Override public List<ShellFile> files(String inRemoteFilePath)
    {
		try {
			return getFiles(
    ...
    public boolean download(
            String remoteFilename, OutputStream to) {
        return download(
                remoteFilename,
                to, to.toString(), null);
    }
		...
		try {
            ...
            if (destinationFile == null) {
                CountingOutputStream counter =
                        new CountingOutputStream( to );

                success = client().retrieveFile(
                        remoteFilename, counter);
            ...
            LOG.debug("Downloaded " + ShellUtils.transferDetails(
                         length, timer.timingMillis()));

            return success;
		} catch (IOException e) {
			LOG.error("Download failed " +
						 remoteFilename + " -> " + destinationName, e);
			return false;
		}
    }
