...
public class TransferSched
{
    ...
    private static final Logger LOG =
            Logger.getLogger(TransferSched.class);
    ...
    //--------------------------------------------------------------------
    public TransferSched(
            final LoginCredential immediateHost,
    ...
    {
        Collection<PendingTransfer> transfers =
                new ArrayList<PendingTransfer>();
        for (String localFile : localFilePaths) {
            transfers.add(new PendingTransfer(
                    localFile,
                    remoteFolder + "/" +
                        AoFiles.name(localFile)));
        }
        uploadAll(transfers);
    }

    public void uploadAll(
            Collection<? extends PendingTransfer> transfers)
    {
    ...
    public void downloadAll(
            ...
            Collection<? extends ShellFile> remoteFiles)
    {
        List<String> remoteFileNames = new ArrayList<String>();
        for (ShellFile fileInfo : remoteFiles) {
            remoteFileNames.add( fileInfo.path() );
        }
        downloadAll(localDirectory,
                    remoteFileNames.toArray(
                            new String[ remoteFileNames.size() ]));
    }
